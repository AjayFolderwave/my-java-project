...
public class ProfileDB {
    ...
    private static String insert_sendGridContact;
    ...
    static {
        try (InputStream input = new FileInputStream("src/main/java/org/example/application.properties")) {
            ...
            insert_sendGridContact = properties.getProperty("jdbc.insert_sendGridContact");
            teamsWebhookUrl = properties.getProperty("teams.webhook_url");
            sqlQuery = properties.getProperty("profiledb.sql_query");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static String createExportJob() {
        ...
        String requestBody = "{ \"list_ids\": [\"f0a45298-270e-496e-9595-d1ba1ae937d1\"] }";
//                                               f0a45298-270e-496e-9595-d1ba1ae937d1
    ...
    private static void processCSV(String csvFileName) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(csvFileName)));
             ...
             PreparedStatement statement = connection.prepareStatement(insert_sendGridContact)) {
                ...
                if (hasResults) {
                    try (ResultSet resultSet = statement.getResultSet()) {
                        while (resultSet.next()) {
                            String idocId = resultSet.getString("IDOCID");
                            String submitDate = resultSet.getString("SUBMITDATE");
                            String cbFinAidId = resultSet.getString("CBFINAIDID");
                            results.append(String.format("Record %d:%n", recordCount + 1));
                            results.append(String.format("**IDOCID**: %s%n", idocId));
                            results.append(String.format("**SubmitDate**: %s%n", submitDate));
                            results.append(String.format("**CBFinAidID**: %s%n%n", cbFinAidId));
                            recordCount++;
                        }
                    }
                }
